---
date: "2023-08-24"
description: curl command description
published: true
tags:
- command
- linux
...
SSL 연결을 위한 `-k, --insecure` 옵션과 `-b, --cookie` 쿠키값 지정을 통해 원하는 호출을 할 수 있습니다.
```shell
$> curl -k --cookie "ARRAffinity=41ee36d2553ab86f8591533e8d88ac03605e73912cbf43c398f1a5ac371d0989" https://www.geeksaga.com/
```

## curl http 활용
```shell
# 프로토콜을 지정하지 않은 경우 기본값은 HTTP로 설정됩니다.
$> curl www.geeksaga.com
...

# HTTP 요청하는 방법을 지정하려면 -X 옵션을 시용할 수 있습니다.
# 기본값은 GET 방식이며 이 방법은 요청할때 별도의 data(Body)가 필요 없습니다.
$> curl -X GET www.geeksaga.com

# -d 옵션으로 전송할 데이터를 나타냅니다.
# -H 옵션으로 전송할 헤더를 지정 합니다.(기본값 : application/x-www-form-urlencoded)
# -X 옵션으로 POST 메소드를 지정하고 요청할 url을 적어 줍니다.
$> curl -d "key1=value1&key2=value2" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -X PORT http://127.0.0.1/post

# 이를 응용하면 slack에 메시지를 보낼 수 있게 됩니다.
$> curl -X POST --data-urlencode "payload={\"channel\": \"채널명\", \"username\": \"Event\", \"text\": \"curl을 통한 메시지 전송 테스트 입니다.\", \"link_names\": 0}" https://hooks.slack.com/services/T0442MF12/XXXXX
```

## help curl
curl 옵션은 하이픈 하나(-)로 시작하는 short 형식과 하이픈 두개(--)로 시작하는 long 형식의 옵션이 있습니다. 
...
Usage: curl [options...] <url>
 ...
 -1, --tlsv1           Use TLSv1.0 or greater
     ...
     --trace-ids       Add transfer/connection identifiers to trace/verbose output
...
[2]: https://man.archlinux.org/man/curl.1 "man curl"
[3]: https://daniel.haxx.se/blog/2023/07/19/curl-8-2-0/ "CURL 8.2.0"
